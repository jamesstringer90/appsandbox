...
static void plant_firstboot_service(ext4_writer_t *ew)
{
    ...
    const char *firstboot_sh =
        ...
        "KB_VARIANT=\n"
        "if [ -s /etc/appsandbox-keyboard-variant ]; then KB_VARIANT=$(tr -d '\\r\\n[:space:]' < /etc/appsandbox-keyboard-variant); fi\n"
        "IM=\n"
        "if [ -s /etc/appsandbox-input-method ]; then IM=$(tr -d '\\r\\n[:space:]' < /etc/appsandbox-input-method); fi\n"
        ...
        "EOF\n"
        "KB_ID=$KB\n"
        "if [ -n \"$KB_VARIANT\" ]; then KB_ID=\"$KB+$KB_VARIANT\"; fi\n"
        "echo \"OK: keyboard=$KB_ID\"\n"
        "rm -f /etc/appsandbox-locale /etc/appsandbox-keyboard /etc/appsandbox-keyboard-variant /etc/appsandbox-input-method 2>/dev/null || true\n"
        "\n"
        ...
        "    IM_PACKAGE=\n"
        "    case \"$IM\" in\n"
        "        libpinyin) IM_PACKAGE=ibus-libpinyin ;;\n"
        "        chewing) IM_PACKAGE=ibus-chewing ;;\n"
        "        mozc-jp) IM_PACKAGE=ibus-mozc ;;\n"
        "        hangul) IM_PACKAGE=ibus-hangul ;;\n"
        "    esac\n"
        "    if [ -n \"$IM_PACKAGE\" ] && [ \"$(dpkg-query -W -f='${Status}' \"$IM_PACKAGE\" 2>/dev/null)\" = 'install ok installed' ]; then\n"
        "        SOURCES=\"[('xkb', '$KB_ID'), ('ibus', '$IM')]\"\n"
        "        MRU=\"[('ibus', '$IM'), ('xkb', '$KB_ID')]\"\n"
        "        echo \"OK: input method=$IM\"\n"
        "    elif [ -n \"$IM\" ]; then\n"
        "        echo \"WARN: input method $IM unavailable; using $KB_ID\"\n"
        "    fi\n"
        "    su \"$ASB_USER\" -c \"dbus-run-session -- gsettings set org.gnome.desktop.input-sources sources \\\"$SOURCES\\\"\" \\\n"
        "      && su \"$ASB_USER\" -c \"dbus-run-session -- gsettings set org.gnome.desktop.input-sources mru-sources \\\"$MRU\\\"\" \\\n"
        "      && echo \"OK: GNOME keyboard=$KB_ID\" || echo \"WARN: GNOME keyboard setup failed\"\n"
        "fi\n"
        ...
        "       libasound2-dev libxcb1-dev libxcb-xfixes0-dev libdrm-dev libsystemd-dev \\\n"
        ...
        "    cd /\n"
        "fi\n"
        "if [ -f /usr/share/gnome-shell/extensions/appsandbox-pointer@appsandbox/metadata.json ]; then\n"
        "    su \"$ASB_USER\" -c \"dbus-run-session -- python3 -\" <<'PY'\n"
        "from gi.repository import Gio\n"
        "settings = Gio.Settings.new('org.gnome.shell')\n"
        "enabled = settings.get_strv('enabled-extensions')\n"
        "uuid = 'appsandbox-pointer@appsandbox'\n"
