                             ...
                             wchar_t *mac_address, size_t str_len)
{
    ...
    PWSTR error_record = NULL;
    const wchar_t *value, *end;
    size_t i;
    HRESULT hr;

    if (!endpoint_guid || !mac_address || str_len < 18)
        return E_INVALIDARG;
    mac_address[0] = L'\0';
    if (!g_hcn_dll || !pfnOpenEp || !pfnQueryEp || !pfnCloseEp)
        return E_NOT_VALID_STATE;

    if (endpoint_guid[0] == L'{') {
        hr = CLSIDFromString(endpoint_guid, &endpoint_id);
    } else {
        if (_snwprintf_s(guid, 40, _TRUNCATE, L"{%s}", endpoint_guid) < 0)
            return E_INVALIDARG;
        hr = CLSIDFromString(guid, &endpoint_id);
    }
    if (FAILED(hr)) return hr;
    ...
    value = properties ? wcsstr(properties, L"\"MacAddress\"") : NULL;
    if (!value) goto done;
    value += wcslen(L"\"MacAddress\"");
    while (iswspace(*value)) value++;
    if (*value++ != L':') goto done;
    while (iswspace(*value)) value++;
    if (*value++ != L'\"') goto done;
    end = wcschr(value, L'\"');
    if (!end || end - value != 17) goto done;
    for (i = 0; i < 17; i++) {
        if (i % 3 == 2) {
            if (value[i] != L'-' && value[i] != L':') goto done;
        } else if (!iswxdigit(value[i])) {
            goto done;
        }
    }
    wcsncpy_s(mac_address, str_len, value, 17);
    hr = S_OK;

done:
    if (error_record) CoTaskMemFree(error_record);
    if (properties) CoTaskMemFree(properties);
    if (endpoint) pfnCloseEp(endpoint);
    return hr;
}
